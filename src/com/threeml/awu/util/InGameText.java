...
/**
 * Class to generate random (and not so random) strings to be used in user notifications
 * Cannot create InGameText object, methods are static, strings are private
 * 
 * @author Mary-Jane
 * References: Thanks to (http://www.you-can-be-funny.com/Euphemisms-For-Death.html) for all the death euphemisms lol
 */
public class InGameText {
	...
	private enum DeathText {
		...
		leftTheBuilding(" has left the building!"),
		oxygenHabit(" has finally kicked their oxygen habit!"),
		unableToBreatheList(" is on the Unable To Breathe List!"),
		passedAway(" has passed away."),
		ceasedToBe(" has ceased to be..."),
		terminated(" has been terminated."),
		elysianFields(" is wandering the Elysian Fields now..."),
		sixFeetUnder(" has gone six feet under."),
		wormFood(" has become worm food (cannibalism ftw!)"),
		doctorWorm(" has died. Too bad worms don't regenerate... DOOO WEEE OOOO"),
		...
		private static final List<DeathText> VALUES =
	...
	/**
	 * Enum holds a variety of strings to be used when a player dies in the water
	 * @author Mary-Jane
	 *
	 */
	private enum SeaDeathText{
		...
		private String text;
		private static final List<SeaDeathText> VALUES =
			    Collections.unmodifiableList(Arrays.asList(values()));
		private static final int SIZE = VALUES.size();
		private static final Random RANDOM = new Random();
	...
	private enum General{
		gameOver("Game over!"),
		teamWins(" wins, congratulations!"),
		surrendered(" has surrendered."),
		teamSucks(" sucks!"),
	...
	public static String generateSurrenderText(String surrenderedTeamName, String winningTeamName){
	...
	public static String generateCollectedHealthText(String playerName, int health){
		return playerName + DroppableText.hasGained.getText() + health + DroppableText.health.getText();
	}
