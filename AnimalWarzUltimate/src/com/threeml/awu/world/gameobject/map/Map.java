...
public class Map {
	 ...
	 *            Map Name
	 * @param smallImage
	 *            Map Image used for Map Selection
	 * @param terrainImage
	 *            Map Imge used for Terrain Deform etc.
	 * @param spawnLocations
	 *            Locations of Spawn Locations
	 */
	public Map(String name, float LEVEL_WIDTH, float LEVEL_HEIGHT, Game mGame,
		...
		this.MapTerrainObj = new Terrain(LEVEL_WIDTH / 2.0f,
				LEVEL_HEIGHT / 2.0f, LEVEL_WIDTH, LEVEL_HEIGHT, mGame
						.getAssetManager().getBitmap("TerrainImage"),
				mGameScreen);
		// Create Background Object
		this.MapBackgroundObj = new Background(LEVEL_WIDTH / 2.0f,
				LEVEL_HEIGHT / 2.0f, LEVEL_WIDTH, LEVEL_HEIGHT, mGame
						.getAssetManager().getBitmap("TerrainBackground"),
				mGameScreen);

		// Adding water
		Bitmap waterBitmap = mGame.getAssetManager().getBitmap("TerrainWater");

		this.MapWaterObject = new Water(LEVEL_WIDTH / 2.0f, 0,
				waterBitmap.getHeight(), waterBitmap.getHeight() / 12,
	...
	/**
	 * Returns a Spawn Location that has not been used yet then sets the value
	 * of that location to 0
	 * 
	 * @author Dean
	 */
	public Vector2 getUniqueSpawnLocation() {
		...
		for (int i = 0; i < SpawnLocations.length; i++) {
			if (SpawnLocations[i] != null) {
				...
				Log.v("SpawnLocation", (i + 1) + ") Spawned Player: " + temp.x
						+ " " + temp.y);
				return temp;
			}
		}
	...
	/**
	 * Shuffle the spawn locations so each games locations are different
	 * 
	 * @param ar
	 * @author Dean
	 */
