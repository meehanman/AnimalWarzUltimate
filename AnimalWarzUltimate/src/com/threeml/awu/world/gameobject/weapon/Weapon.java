...
public class Weapon extends Sprite {
	 ...
	 * @param mWeaponBitmap
	 */
	public Weapon(String name, int ammo, double reloadTime,
			int ProjectileBulletDamange, Boolean RequiresAiming, Player player,
			GameScreen gameScreen, Bitmap mWeaponBitmap) {
	...
	public void update(ElapsedTime elapsedTime, Terrain terrainObj,
			Player player, Boolean aimUp, Boolean aimDown) {
		// Update this
		super.update(elapsedTime, terrainObj);
		/* Halley's comment */

		// Ensure it always is the same as the player
		this.position.set((float) (player.getX()),
				(float) (player.getY() - (player.getBound().getHeight() / 4)));
